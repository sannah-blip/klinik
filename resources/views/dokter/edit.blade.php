...
@section('header_title', 'Perbarui Data Dokter')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between pb-4 border-b border-slate-200">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Dokter Spesialis</h1>
            <p class="text-sm text-slate-500 mt-1">Ubah formulir di bawah untuk memperbarui data dokter.</p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="datetime-local"
                           name="jadwal_mulai"
                           value="{{ old('jadwal_mulai', $dokter->jadwal_mulai ? \Carbon\Carbon::parse($dokter->jadwal_mulai)->format('Y-m-d\TH:i') : '') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local"
                           name="jadwal_selesai"
                           value="{{ old('jadwal_selesai', $dokter->jadwal_selesai ? \Carbon\Carbon::parse($dokter->jadwal_selesai)->format('Y-m-d\TH:i') : '') }}"
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-sm rounded-xl text-slate-700 focus:outline-none focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-50 transition-all duration-200" required>
                </div>
            </div>
